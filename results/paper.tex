...
\toprule
\textbf{Scale Tier} & \textbf{Controllers ($M$)} & \textbf{Switches ($N$)} & \textbf{Hosts ($X$)} & \textbf{Pass Rate} & \textbf{Packet Loss (\%)} & \textbf{Mean Recovery Latency (s)} \\
\midrule
...
As detailed in Table~\ref{tab:adg_scale_results}, zero packet loss (0.0\%) was achieved across all evaluation scenarios. Fig.~\ref{fig:scale_failover} visualizes the mean failover recovery latency with error bars across all four topology configurations.

\begin{figure}[h]
\centering
\includegraphics[width=\linewidth]{scale_failover_latency.png}
\caption{Mean failover recovery latency across SDN scale tiers. Error bars represent $\pm 1\sigma$ standard deviation over 5 runs per tier. All configurations achieved 0\% packet loss.}
...
\subsection{Multi-Controller Coordination}
While the prototype now requests \texttt{OFPCR\_ROLE\_EQUAL} for basic multi-controller support, full multi-controller coordination---including state synchronization across distributed controllers and consensus-based failover---remains future work.

\subsection{Response Mechanics and Performance Benchmarking}
Future work will extend the enforcement plane to support honeypot redirection~\cite{provos2004virtual}, automated deception via moving target defence~\cite{jajodia2011mtd}, and Weighted Max-Min Fairness (WMMF) bandwidth allocation for gradual throttling as an alternative to binary DROP. Additionally, quantitative evaluation of detection latency, CPU/memory overhead, throughput impacts, and false-positive rates under varying network loads remains an essential step for production deployment.
...
This paper presented the current implementation of the Active Defense Gateway (ADG), an eBPF-assisted adaptive trust enforcement framework for Software Defined Networks.
...
The Trust Engine was independently validated through nine Rust unit tests, all of which passed. The Policy Engine was validated through eleven boundary-condition tests. Additional integration tests verified escalation and recovery behaviour across the ALLOW $\rightarrow$ MIRROR $\rightarrow$ DROP $\rightarrow$ ALLOW lifecycle, controller mock tests validated PacketIn handling and EQUAL role negotiation, and trust store tests confirmed correct BPF map lookup behaviour. In total, 51 automated tests across Rust and Python components confirm the system's deterministic behaviour. System-level testing demonstrated successful XDP attachment, host telemetry collection, trust-state propagation, controller-side trust evaluation, and adaptive policy transition.

The windowed delta telemetry mechanism enables autonomous trust recovery: when malicious traffic ceases, the behavioural profile naturally clears, restoring full trust without manual intervention.
...
The present implementation is intentionally limited to a small Mininet topology. Consequently, claims regarding large-scale scalability, distributed controller resilience, automated deception, and production-level mitigation are outside the scope of the current evaluation.
