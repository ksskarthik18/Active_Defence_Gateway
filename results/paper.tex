...
A comprehensive audit of the observation plane revealed that the XDP program currently parses but does not store several security-relevant fields: destination IP addresses, TCP destination ports, and TCP RST/FIN/ACK flags. These fields are available in the \texttt{Ipv4Hdr} and \texttt{TcpHdr} structures but are not written to \texttt{HOST\_STATS}. Extending the BPF map to include these fields would enable port scan detection (via destination port diversity), lateral movement detection (via destination IP diversity), and failed connection analysis (via RST:SYN ratios).

\subsection{Planned Extensions}
The following extensions are prioritized for near-term development:
\begin{itemize}
\item \textbf{Destination intelligence}: tracking unique destination IPs and ports per host to detect scanning and lateral movement patterns.
\item \textbf{TCP RST tracking}: extracting RST flags from the existing TCP header parser to identify failed connection ratios characteristic of port scans.
...
\item \textbf{Packet size statistics}: tracking min/max packet sizes to identify uniform small packets (scan signatures) or uniform large packets (exfiltration indicators).
\end{itemize}
